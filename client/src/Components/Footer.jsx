        ...
        <div className="flex flex-col md:flex-row lg:flex-row justify-around my-5 md:my-0 lg:my-0">
          <div>
            ...
            <ul className="flex flex-col">
              {Navlinks.map((link) => (
                <NavLink key={link.name} to={link.path} className={`${({ isActive }) => isActive ? "text-(--color-brand-primary)" : ""} text-[13px] text-(--text-secondary) my-2 hover:text-(--color-brand-primary)`}>{link.name}</NavLink>
              ))}
            </ul>
          </div>
          ...
          <div>
            ...
            <ul className="flex flex-col">
              {supportPages.map((page) => (
                <Link key={page.name} to={page.path} className="text-[13px] text-(--text-secondary) my-2 hover:text-(--color-brand-primary)">{page.name}</Link>
              ))}
            </ul>
          </div>
        </div>
