...
const ShopPage = ({ products = [], categories = [] }) => {
  ...
  const filteredProducts = useMemo(() => {
    ...
    return filtered;
  }, [products, selectedCategories, selectedBrands, priceRange, showInStock, sortBy]);
              ...
              {categories.map((category) => (
                <label key={category.id} className="flex items-center justify-between cursor-pointer group">
                  <div className="flex items-center gap-2">
                    <input
                      type="checkbox"
                      checked={selectedCategories.includes(category.slug?.toLowerCase())}
                  ...
                  <span className="text-xs text-(--text-secondary)">({category._count?.products || 0})</span>
                </label>
              ))}
            </div>
          )}
            ...
            <div className="lg:col-span-3">
              {filteredProducts.length > 0 ? (
                ...
                <div className="bg-white border border-(--border-default) rounded-xl p-12 text-center">
                  <p className="text-(--text-secondary) text-lg mb-4">No products found matching your filters</p>
                  <button
                    onClick={clearAllFilters}
