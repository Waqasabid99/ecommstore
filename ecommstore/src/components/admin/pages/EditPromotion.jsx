...
const EditPromotion = () => {
  ...
  const { adminID, promotionId: id } = useParams();
  ...
  const fetchPromotion = async () => {
    try {
      setIsLoading(true);
      const { data } = await axios.get(`${baseUrl}/promotions/${id}`, {
      ...
      if (data.success) {
        ...
        setFormData({
          ...
          endsAt: formatDateTime(promotion.endsAt),
          ...
          variantIds: promotion.variants?.map((v) => v.id) || [],
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const { data } = await axios.put(`${baseUrl}/promotions/${id}`, payload, {
        withCredentials: true,
      });
      ...
      const errorMessage =
        error.response?.data?.error || "Failed to update promotion";
      setNotification({
        type: "error",
      ...
      <div className="bg-white rounded-xl max-w-md w-full p-8">
        <div className="text-center">
          ...
          <p className="text-sm text-gray-500 mb-6">
            Are you sure you want to delete this promotion? This action will
  ...
  return (
    <div className="min-h-screen bg-[#F8F8F8]">
      ...
      <DashboardHeadingBox
        text="Edit Promotion"
        subHeading="Update promotion details and settings"
        button={
          <>
            <button
              onClick={() => setShowDeleteModal(true)}
              className="bg-red-500 text-white rounded font-semibold p-3 border border-transparent hover:bg-red-600 transition-colors flex items-center gap-2"
            >
              <Trash2 size={16} />
              Delete
            </button>
                ...
                <div>
                  ...
                  <div className="bg-gray-50 border border-gray-200 rounded-lg p-4">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="font-medium">{formData.appliesTo}</span>
                    </div>
                    <p className="text-xs text-gray-500 mt-2">
                      The application type cannot be changed after creation.
                    </p>
                  </div>
                </div>
