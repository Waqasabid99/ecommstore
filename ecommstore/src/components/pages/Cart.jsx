...
import { Trash2, Plus, Minus, ShoppingBag, ArrowRight, Tag, Truck, Shield, CreditCard, X } from 'lucide-react';
import Link from 'next/link';

// Sample cart items - in production, this would come from your state management
        ...
        <section className="mx-4 my-6">
          <div className="max-w-7xl mx-auto">
            <div className="bg-white border border-(--border-default) rounded-xl p-12 md:p-20 text-center">
              ...
              <h2 className="text-3xl font-bold mb-4 text-(--text-heading)">Your Cart is Empty</h2>
      ...
      <section className="bg-linear-to-br from-blue-50 to-purple-50 mx-4 rounded-xl px-6 py-12 md:py-16 mb-6">
        <div className="max-w-7xl mx-auto">
          <div className="flex items-center justify-between flex-wrap gap-4">
            ...
            <button className="text-(--text-secondary) hover:text-(--text-hover) font-medium flex items-center gap-2">
          ...
          <div className="grid lg:grid-cols-3 gap-6">
            ...
            <div className="lg:col-span-2 space-y-4">
              ...
              <div className="bg-[#FFF5F5] border border-(--border-default) rounded-xl p-6">
                <div className="flex items-center gap-2 mb-4">
                  ...
                  <h3 className="font-semibold text-(--text-heading)">Have a Promo Code?</h3>
                </div>
                ...
                {appliedPromo ? (
                  <div className="flex items-center justify-between bg-white border border-(--border-primary) rounded-lg p-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="font-semibold text-(--text-heading)">{appliedPromo}</p>
                        <p className="text-sm text-(--color-brand-primary)">10% discount applied</p>
                      </div>
                    </div>
                    <button
                ...
                <p className="text-xs text-(--text-secondary) mt-3">
                  Try code: <span className="font-semibold text-(--color-brand-primary)">SAVE10</span> for 10% off
                </p>
              </div>
            </div>
            ...
            <div className="lg:col-span-1">
              <div className="bg-white border border-(--border-default) rounded-xl p-6 sticky top-6">
                <h2 className="text-2xl font-bold mb-6 text-(--text-heading)">Order Summary</h2>
                ...
                <Link href={"/checkout"} className="w-full bg-(--btn-bg-primary) text-(--btn-text-primary) px-6 py-4 rounded-full hover:bg-(--btn-bg-hover) transition-all font-medium text-lg mb-4 flex items-center justify-center gap-2">
                  Proceed to Checkout
                  <ArrowRight size={20} />
                </Link>

                <button className="w-full border-2 border-(--border-inverse) text-(--text-primary) px-6 py-3 rounded-full hover:bg-(--bg-inverse) hover:text-(--text-inverse) transition-all font-medium">
                  Continue Shopping
                </button>
                ...
                <div className="mt-6 pt-6 border-t border-(--border-default) space-y-4">
                  <div className="flex items-start gap-3">
                    <div className="w-10 h-10 bg-(--bg-surface) rounded-full flex items-center justify-center shrink-0">
                      <Truck size={18} className="text-(--color-brand-primary)" />
                    </div>
                    <div>
                      <p className="font-medium text-sm text-(--text-heading)">Free Shipping</p>
                      <p className="text-xs text-(--text-secondary)">On orders over Rs. 50,000</p>
                    </div>
                  </div>

                  <div className="flex items-start gap-3">
                    <div className="w-10 h-10 bg-(--bg-surface) rounded-full flex items-center justify-center shrink-0">
                      <Shield size={18} className="text-(--color-brand-primary)" />
                    </div>
                    <div>
                      <p className="font-medium text-sm text-(--text-heading)">Secure Payment</p>
                      <p className="text-xs text-(--text-secondary)">100% secure transactions</p>
                    </div>
                  </div>

                  <div className="flex items-start gap-3">
                    <div className="w-10 h-10 bg-(--bg-surface) rounded-full flex items-center justify-center shrink-0">
                      <CreditCard size={18} className="text-(--color-brand-primary)" />
                    </div>
                    <div>
                      <p className="font-medium text-sm text-(--text-heading)">Easy Returns</p>
